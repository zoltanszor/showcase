//FJSTARTHEADER
// $Id: fjcorefortran.cc 4420 2019-11-29 09:28:20Z soyez $
//
...
//  jets = sorted_by_pt(cs->inclusive_jets(0e0));

    // transfer jets -> f77jets[4*ijet+0..3]
//  transfer_jets(f77jets, njets);
    
//MOD by Zoltan to do Ecut manually
    jets = cs->inclusive_jets();
    double tmp_size = 0;
    for(int i=0;i<jets.size();i++){
      if(jets[i].E()>8e0) tmp_size++;
    }
//    njets = jets.size();
    njets = tmp_size;
...
//  transfer_cluster_transfer(p,npart,jet_def,f77jets,njets);
    transfer_cluster_transfer(p,npart,jet_def,njets);
}
